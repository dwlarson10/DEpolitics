...
tweets <- twListToDF(userTimeline(user = "JohnCarneyDE",includeRts = TRUE,n=5000))
...
dayofweek <- ggplot(tweets)+
        ...
        ggtitle("@JohnCarneyDE Tweets by day of week and hour of day") +
...
ggsave(dayofweek,file="JCdayofweek.png",width = 10,height = 10)
...
plotDays <- ggplot(data=day,aes(x=date)) +
        geom_line(aes(y=tweets),size=1,colour = "#00b5ab") +
        ...
        theme(legend.position="none") +
        # Set title and axis labels, and format these and tick marks
        ggtitle("@JohnCarneyDE tweets per day") +
        theme(plot.title=element_text(face="bold",hjust=0,vjust=2,colour="#3C3C3C",size=20)) +
        ylab("Frequency") +
        xlab("Date") +
        theme(axis.text.x=element_text(size=11,colour="#535353",face="bold")) +
...
ggsave(plotDays,file="JCdays.png",width = 10,height = 10)
...
TorR<- ggplot(data=retweet,aes(x=isRetweet,y=tweets)) +
        ...
        theme(legend.position="none") +
        # Set title and axis labels, and format these and tick marks
        ggtitle("@JohnCarneyDE Tweet or Retweet") +
        theme(plot.title=element_text(face="bold",hjust=0,vjust=2,colour="#3C3C3C",size=20)) +
        ylab("Frequency") +
        xlab("type") +
...
ggsave(TorR,file="JCTorT.png",width = 10,height = 10)
...
png(file="JCthm.png")
twitterheatmap <- calendarHeat(ts.sum.df$date, ts.sum.df$sum, varname="@JohnCarneyDE Twitter activity")
dev.off()
...
sentiment<- ggplot(data=tscore,aes(x=score,y=tweets)) +
        ...
        theme(legend.position="none") +
        # Set title and axis labels, and format these and tick marks
        ggtitle("Sentiment Scores of @JohnCarneyDE Tweets") +
        theme(plot.title=element_text(face="bold",hjust=0,vjust=2,colour="#3C3C3C",size=20)) +
        ylab("Frequency") +
...
ggsave(sentiment,file="JCsentiment.png",width = 10,height = 10)