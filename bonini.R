...
tweets <- twListToDF(userTimeline(user = "ColinBonini",includeRts = TRUE ,n=5000))
...
#tweets <- subset(tweets,tweets$year == 2016)
...
dayofweek <- ggplot(tweets)+
        ...
        ggtitle("@ColinBonini Tweets by day of week and hour of day") +
...
ggsave(dayofweek,file="CBdayofweek.png",width = 10,height = 10)

daybase <- tbl_df(data.frame(seq.Date(from = as.Date('2016-01-01'),to = as.Date('2016-11-05'),by = 'day')))
names(daybase) <- 'date'
...
plotDays <- ggplot(data=day,aes(x=date)) +
        ...
        theme(legend.position="none") +
        # Set title and axis labels, and format these and tick marks
        ggtitle("@ColinBonini tweets per day") +
        theme(plot.title=element_text(face="bold",hjust=0,vjust=2,colour="#3C3C3C",size=20)) +
        ylab("Frequency") +
        xlab("Date") +
        theme(axis.text.x=element_text(size=11,colour="#535353",face="bold")) +
...
ggsave(plotDays,file="CBdays.png",width = 10,height = 10)
...
TorR<- ggplot(data=retweet,aes(x=isRetweet,y=tweets)) +
        ...
        theme(legend.position="none") +
        # Set title and axis labels, and format these and tick marks
        ggtitle("@ColinBonini Tweet or Retweet") +
        theme(plot.title=element_text(face="bold",hjust=0,vjust=2,colour="#3C3C3C",size=20)) +
        ylab("Frequency") +
        xlab("type") +
...
ggsave(TorR,file="CBTorT.png",width = 10,height = 10)
...
png(file="cbthm.png")
Carneyheatmap <- calendarHeat(ts.sum.df$date, ts.sum.df$sum, varname="@ColinBonini Twitter activity")
dev.off()
...
wc<-wordcloud(corpus, min.freq=5, scale=c(5,2),rot.per = 0.25,
...
tweets$score=as.numeric(score)

tscore <- tbl_df(tweets)%>%group_by(score)%>%summarise(tweets=n())
...
sentiment<- ggplot(data=tscore,aes(x=score,y=tweets)) +
        ...
        theme(legend.position="none") +
        # Set title and axis labels, and format these and tick marks
        ggtitle("Sentiment Scores of @ColinBonini Tweets") +
        theme(plot.title=element_text(face="bold",hjust=0,vjust=2,colour="#3C3C3C",size=20)) +
        ylab("Frequency") +
...
ggsave(sentiment,file="CBsentiment.png",width = 10,height = 10)
